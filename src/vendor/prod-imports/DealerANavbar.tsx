/**
 * DealerANavbar · demo topbar mirroring the production Expert Hub +
 * quote-converter Navbars, grafted for the Dealer A demo context.
 *
 * SOURCES:
 ...
 *   Feedback + PDF-to-SIF as standard modules across BOTH Dealer A
 *   experiences (Expert Hub + Dealer). Dealer A demo topbar was the
...
// broke because it depends on the `dealer-a:w9-open` event the AC dispatches).
...
type DealerAExperience = 'expert-hub' | 'dealer'

interface DealerANavbarProps {
...
const EXPERT_HUB_TABS: Tab[] = [
    { name: 'OCR',          label: 'OCR Tracking', page: 'dealer-a-ocr',           icon: ScanEye  },
    { name: 'Transactions', label: 'Transactions', page: 'dealer-a-transactions',  icon: Receipt  },
    { name: 'Comparisons',  label: 'Comparisons',  page: 'dealer-a-comparisons',   icon: GitCompare },
    { name: 'PDtoSIF',      label: 'PD to SIF',    page: 'dealer-a-pd-to-sif',     icon: FileText },
]

const DEALER_TABS: Tab[] = [
    { name: 'OCR',           label: 'OCR',           page: 'dealer-a-ocr',           icon: ScanEye  },
    { name: 'Observability', label: 'Observability', page: 'dealer-a-observability', icon: Activity },
    { name: 'Transactions',  label: 'Transactions',  page: 'dealer-a-transactions',  icon: Receipt  },
    { name: 'PDtoSIF',       label: 'PD to SIF',     page: 'dealer-a-pd-to-sif',     icon: FileText },
]

export default function DealerANavbar({
    experience,
    activeTab,
    onNavigate,
    onOpenFeedback,
    ...
    // the same pattern lets account teams switch demos (Dealer A ↔ BFI ↔
    // Officeworks · etc.) without leaving the Dealer A chrome.
            ...
            <div className="relative flex items-center lg:justify-between px-3 py-2 rounded-full gap-1 bg-card/80 backdrop-blur-xl border border-border shadow-lg">
                ...
                <div className="flex items-center gap-1">
                    ...
                    {/* F85.4 · Demo profile Popover · same shape as shared Navbar's
                        ...
                        listing every profile (Dealer A, BFI, Officeworks, etc.). */}
                ...
                <div className="flex items-center gap-1 shrink-0">
                    ...
                    {/* F85.2 · Action Center · restored after F85.1 regression.
                        ...
                        DemoContext · fires dealer-a:* events on CTA click. */}
                    <ActionCenter />
                    ...
                    <div className="relative" ref={feedbackRef}>
                        ...
                        {isFeedbackMenuOpen && (
                            ...
                            >
                                ...
                                <button
                                    role="menuitem"
                                    onClick={() => { setIsFeedbackMenuOpen(false); onNavigate('dealer-a-feedback-status') }}
                                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted rounded-lg transition-colors"
                                >
