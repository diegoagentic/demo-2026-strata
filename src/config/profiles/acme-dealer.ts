...
//
// CLIENT: Acme Dealer Inc. (Denver + Seattle · ~25 employees · 3 legal entities
//         Acme Dealer Inc / Acme Dealer Corp / Culture LLC · commercial furniture +
...
export const ACME_DEALER_STEPS: DemoStep[] = [
    ...
    // ═══════════════════════════════════════════
    {
        ...
        description: 'Strata OCR sweeps bills@acme-dealer.com overnight and files 14 vendor bills in the queue. Teknion NCBA + Warehouse-by-Design flagged for review.',
        app: 'acme-dealer-bills',
        role: 'Senior Accountant',
        flowId: 'acme-dealer-bills',
    },
    {
        id: 'p1.2',
        groupId: 1,
    ...
    // ═══════════════════════════════════════════
    {
        ...
        app: 'acme-dealer-vendor-onboarding',
        role: 'Furniture Coordinator',
        flowId: 'acme-dealer-vendor-onboarding',
    },
    {
        id: 'p2.2',
        groupId: 2,
    ...
    // ═══════════════════════════════════════════
    {
        ...
        app: 'acme-dealer-billing',
        role: 'Furniture Coordinator',
        flowId: 'acme-dealer-billing',
    },
    {
        id: 'p3.2',
        groupId: 3,
    ...
    // ═══════════════════════════════════════════
    {
        ...
        app: 'acme-dealer-order-po',
        role: 'Furniture Coordinator',
        flowId: 'acme-dealer-order-po',
    },
    {
        id: 'p4.2',
        groupId: 4,
...
export const ACME_DEALER_STEP_BEHAVIOR: Record<string, StepBehavior> = {
    // F1 · AP
...
export const ACME_DEALER_STEP_MESSAGES: Record<string, string[]> = {
    'p1.1': [
        'Email Capture watching bills@acme-dealer.com · 14 bills ingested overnight',
...
export const ACME_DEALER_SELF_INDICATED: string[] = [
    'p1.1', // OCR ingest
    'p1.2', // AI comparison
...
export type AcmeDealerExperience = 'expert-hub' | 'dealer';

export type AcmeDealerFlowId =
    | 'acme-dealer-bills'
    | 'acme-dealer-vendor-onboarding'
    | 'acme-dealer-billing'
    | 'acme-dealer-order-po'
    | 'acme-dealer-ack';

export const ACME_DEALER_EXPERIENCE_MAP: Record<AcmeDealerFlowId, AcmeDealerExperience> = {
    'acme-dealer-bills':                'expert-hub',   // F1 · Bills intake & matching
    'acme-dealer-vendor-onboarding': 'dealer',       // F2 · Vendor onboarding & compliance
    'acme-dealer-billing':           'dealer',       // F3 · Progress billing & collections
    'acme-dealer-order-po':          'expert-hub',   // F4 · Order entry & PO dispatch
    'acme-dealer-ack':               'expert-hub',   // F5 · Electronic ordering & ACK processing
};

/** Returns 'expert-hub' | 'dealer' for a given flow id. */
export function experienceOf(app: string | undefined): AcmeDealerExperience {
    if (!app) return 'expert-hub';
    return ACME_DEALER_EXPERIENCE_MAP[app as AcmeDealerFlowId] ?? 'expert-hub';
}

/** Grouped view for the 2-tab segmented switcher. */
export const ACME_DEALER_EXPERIENCE_GROUPS: {
    id: AcmeDealerExperience;
    label: string;
    flows: { id: AcmeDealerFlowId; label: string; short: string }[];
}[] = [
    {
        id: 'expert-hub',
        label: 'Expert Hub',
        flows: [
            { id: 'acme-dealer-bills',        label: 'Bills intake & matching',                short: 'F1 · AP' },
            { id: 'acme-dealer-order-po',  label: 'Order entry & PO dispatch',           short: 'F4 · Order/PO' },
            { id: 'acme-dealer-ack',       label: 'Electronic ordering & ACK',           short: 'F5 · ACK' },
        ],
    },
    {
        id: 'dealer',
        ...
        flows: [
            { id: 'acme-dealer-vendor-onboarding', label: 'Vendor onboarding & compliance', short: 'F2 · Vendor onboarding' },
            { id: 'acme-dealer-billing',           label: 'Progress billing & collections', short: 'F3 · Billing' },
        ],
    },
];

/** First flow id of a given experience. */
export function firstFlowOf(experience: AcmeDealerExperience): AcmeDealerFlowId {
    return ACME_DEALER_EXPERIENCE_GROUPS.find(g => g.id === experience)!.flows[0].id;
}

// ─── PATH LANDINGS (F84 MVP · 3 moments per path · all core) ─────────────────

export interface AcmeDealerPathMoment {
    stepId: string;
    title: string;
    description: string;
    estTime: string;
...
export interface AcmeDealerPathLanding {
    tagline: string;
    valueChip: { label: string; tone: 'success' | 'ai' | 'warning' | 'info' };
    heroStepId: string;
    moments: AcmeDealerPathMoment[];
    ctaGuided: string;
    ctaExplore: string;
}

export const ACME_DEALER_PATH_LANDINGS: Record<AcmeDealerFlowId, AcmeDealerPathLanding> = {
    'acme-dealer-bills': {
...
export const ACME_DEALER_CORE_STEP_IDS: Set<string> = new Set(
    Object.values(ACME_DEALER_PATH_LANDINGS).flatMap(l =>
        l.moments.filter(m => m.isCore).map(m => m.stepId)
    )
);

export function isCoreStep(stepId: string): boolean {
    return ACME_DEALER_CORE_STEP_IDS.has(stepId);
}

// ─── PRESENTER NOTES (F84 · 15 steps · 30-sec script per screen) ─────────────

export interface AcmeDealerPresenterNote {
    say: string;
...
export const ACME_DEALER_PRESENTER_NOTES: Record<string, AcmeDealerPresenterNote> = {
