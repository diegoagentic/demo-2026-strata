...
// ─── Dealer A · step notifications (F74 · parallel to BFI/Officeworks/CLC) ─────
...
type DealerAStepNotif = BfiStepNotif

// F76 · Per-step notif delay override for Dealer A · steps donde la scene UI
...
const DEALER_A_NOTIF_DELAY_MS: Record<string, number> = {
    'p1.1': 3000,
    'p2.1': 3000,
    'p2.2': 3000,
    'p3.1': 3000,
...
const DEALER_A_NOTIF_DEFAULT_DELAY_MS = 2000
...
const DEALER_A_STEP_NOTIFICATIONS: Record<string, DealerAStepNotif> = {
    'p1.1': {
        badge: '14 bills · 3 need eyes', badgeColor: 'ai',
        title: 'F1 · Bills inbox · Strata OCR filed 14 vendor bills overnight',
        desc: 'Overnight sweep of bills@dealer-a.com · 14 vendor bills ingested and filed in the OCR queue. Teknion NCBA (bill 8483) and Warehouse-by-Design install invoice both flagged for review · walkthrough starts here.',
        sender: 'Strata AI · bills@dealer-a.com · sweep 02:14 → 08:11',
        re: 'Bills inbox · 14 bills · 3 legal entities · ready for Compliance',
        cta: 'Open the Bills queue →',
        event: 'dealer-a:bills-open',
...
export default function ActionCenter() {
    ...
    // Dealer A generic step panel (p1.1 .. p5.x · F74)
    const [dealerAPanelClosed, setDealerAPanelClosed] = useState(false);
    // Delay before any notification panel appears (2s after step loads)
    const [notifDelayReady, setNotifDelayReady] = useState(false);
    // F76 · Dealer A-specific delay ready (respects per-step delay override)
    const [dealerANotifReady, setDealerANotifReady] = useState(false);
    ...
    useEffect(() => {
        ...
        setDealerAPanelClosed(false);
        setNotifDelayReady(false);
        setDealerANotifReady(false);
        const cancel = pauseAwareTimeout(() => setNotifDelayReady(true), 2000);
        // Dealer A per-step delay (F3 dashboards need more scan time)
        const dealerADelay = DEALER_A_NOTIF_DELAY_MS[currentStep?.id ?? ''] ?? DEALER_A_NOTIF_DEFAULT_DELAY_MS;
        const cancelDealerA = pauseAwareTimeout(() => setDealerANotifReady(true), dealerADelay);
        return () => { cancel?.(); cancelDealerA?.(); };
    ...
    const dealerAStepConfig = isDemoActive ? DEALER_A_STEP_NOTIFICATIONS[currentStep?.id ?? ''] : undefined;
    const isDealerAStepActive = !!dealerAStepConfig && !dealerAPanelClosed && dealerANotifReady;
    ...
    return (
        ...
        {/* Dealer A Steps p1.1 .. p5.x: Generic incoming-event notification panel (F74) */}
        {isDealerAStepActive && dealerAStepConfig && (
            <div className={clsx("fixed top-[90px] -translate-x-1/2 w-[95vw] lg:w-[520px] z-50 animate-in fade-in slide-in-from-top-2 duration-300", sidebarExpanded ? 'left-[calc(50%+10rem)]' : 'left-1/2')}>
                <div className="bg-zinc-100 dark:bg-zinc-900/85 backdrop-blur-xl border border-border shadow-2xl rounded-3xl overflow-hidden">
                    <div className="px-5 pt-5 pb-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Action Center</h3>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                                dealerAStepConfig.badgeColor === 'warning' ? 'bg-warning/15 text-warning' :
                                dealerAStepConfig.badgeColor === 'success' ? 'bg-success/15 text-success' :
                                'bg-foreground/10 text-foreground'
                            }`}>{dealerAStepConfig.badge}</span>
                        </div>
                        <button onClick={() => setDealerAPanelClosed(true)} className="p-1 rounded-full hover:bg-black/5 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400 transition-colors">
                            <XMarkIcon className="w-5 h-5" />
                        </button>
                    </div>
                    <div className="px-5 pb-5">
                        <div className="rounded-2xl bg-zinc-50 dark:bg-zinc-800 border border-border overflow-hidden">
                            <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
                                <SparklesIcon className="w-4 h-4 text-ai shrink-0" />
                                <span className="text-sm font-semibold text-foreground flex-1">{dealerAStepConfig.title}</span>
                                <span className="text-[10px] text-muted-foreground shrink-0">Today · 08:14 AM</span>
                            </div>
                            <div className="px-4 py-3 border-b border-border space-y-1">
                                <div className="flex gap-2 text-[11px]">
                                    <span className="text-muted-foreground w-10 shrink-0">From</span>
                                    <span className="text-foreground font-medium">{dealerAStepConfig.sender}</span>
                                </div>
                                {dealerAStepConfig.re ? (
                                    <div className="flex gap-2 text-[11px]">
                                        <span className="text-muted-foreground w-10 shrink-0">Re</span>
                                        <span className="text-foreground">{dealerAStepConfig.re}</span>
                                    </div>
                                ) : (
                                    <div className="flex gap-2 text-[11px]">
                                        <span className="text-muted-foreground w-10 shrink-0">Info</span>
                                        <span className="text-foreground">{dealerAStepConfig.desc}</span>
                                    </div>
                                )}
                            </div>
                            {dealerAStepConfig.attachment && (
                                <div className="px-4 py-2.5 border-b border-border flex items-center gap-2">
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wide shrink-0">Attachment:</span>
                                    <span className="flex items-center gap-1 text-[10px] text-success font-medium px-2 py-0.5 rounded bg-success/10 border border-success/20">
                                        <DocumentTextIcon className="w-3 h-3" /> {dealerAStepConfig.attachment}
                                    </span>
                                </div>
                            )}
                            {dealerAStepConfig.re && (
                                <div className="px-4 py-3 border-b border-border">
                                    <p className="text-[11px] text-muted-foreground leading-relaxed">{dealerAStepConfig.desc}</p>
                                </div>
                            )}
                            <div className="px-4 py-4">
                                <button
                                    onClick={() => {
                                        setDealerAPanelClosed(true);
                                        window.dispatchEvent(new CustomEvent(dealerAStepConfig.event));
                                    }}
                                    className="w-full py-2.5 text-[12px] font-black rounded-xl bg-foreground text-background hover:opacity-80 transition-all flex items-center justify-center gap-2 uppercase tracking-widest"
                                >
                                    {dealerAStepConfig.cta}
                                </button>
                            </div>
                        </div>
                    </div>
