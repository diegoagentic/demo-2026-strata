 ...
 * PURPOSE: Auto. Proforma PJX-INV-3421 flips to Customer Invoice. NetSuite GL
 *          entry visible. Activity timeline log. 3-step post sequence
 *          (validating → creating → notifying).
 *
 * SHAPE · staged reveal (3 steps) + GL journal entry preview + activity timeline
 * REUSE · workspaces/GLCoreSyncScene pattern (3-step post con comment threads)
 * NOTIF · dispatchea `projex:invoice-posted` on complete
 */

import { useEffect, useState } from 'react'
import {
    CheckCircle2, Loader2, Database, Sparkles, ArrowRight,
    RotateCcw, Clock, FileText, DollarSign,
} from 'lucide-react'
...
export default function F3_p36_NetSuiteSyncScene() {
    ...
    }, [stepIdx, done, pauseAwareTimeout])
            ...
            <div>
                <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider font-mono text-muted-foreground mb-1">
                    ...
                    <span className="inline-flex items-center gap-1 bg-success/10 text-success font-semibold rounded-md px-1.5 py-0.5">
                        <CheckCircle2 className="h-3 w-3" aria-hidden="true" /> Auto · GL sync
                    </span>
                </div>
