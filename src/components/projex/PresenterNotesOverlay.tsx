 ...
 *          questions + what-to-click-next para el current step Projex. Reduce
 ...
 *          renderea (checks isProjex + isDemoActive + notes disponibles para
 *          el current step).
 */
...
import { PROJEX_PRESENTER_NOTES } from '../../config/profiles/projex'
import { usePersistedToggle } from './hooks/usePersistedToggle'

export default function PresenterNotesOverlay() {
    const { activeProfile } = useDemoProfile()
    const { currentStep, isDemoActive } = useDemo()
    const [visible, setVisible] = usePersistedToggle('projex-presenter-mode', false, 'presenter')
    ...
    // Guards · solo Projex profile · solo con demo activo (los notes son
    // per-step) · solo si hay notes para el step actual
    if (activeProfile.id !== 'projex') return null
    if (!isDemoActive || !currentStep) return null
    const notes = PROJEX_PRESENTER_NOTES[currentStep.id]
