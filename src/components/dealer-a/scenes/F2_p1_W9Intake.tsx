 ...
 *   3. AC notif CTA dispatches `dealer-a:w9-open` · scene opens the prod
 *      DocumentReviewModal · Save/Cancel advances to p2.2.
 *
 * DealerAPage Shell skips its own advance for `dealer-a:w9-open` via
...
export default function F2_p1_W9Intake() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('dealer-a:w9-open', handler)
        return () => window.removeEventListener('dealer-a:w9-open', handler)
    }, [])
