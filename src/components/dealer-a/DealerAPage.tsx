/**
 * COMPONENT: DealerAPage · F84 MVP (2026-08-21)
 * PURPOSE: Route the 15 Dealer A demo steps to their scene component.
 *          One story per path · 3 screens (initial → process → final) ·
 *          all prod-lifted UI · zero decoration.
 *
 * USED BY: App.tsx cases `dealer-a-bills` · `dealer-a-vendor-onboarding` ·
 *          `dealer-a-billing` · `dealer-a-order-po` · `dealer-a-ack`.
 */

import { useEffect } from 'react'
import { useDemo } from '../../context/DemoContext'
import DealerAExperienceShell, { type DealerATab } from './DealerAExperienceShell'
import { experienceOf } from '../../config/profiles/dealer-a'
...
function activeTabFor(app: string | undefined): DealerATab {
    if (app === 'dealer-a-bills')                return 'transactions'
    if (app === 'dealer-a-order-po')          return 'transactions'
    if (app === 'dealer-a-ack')               return 'comparisons'
    if (app === 'dealer-a-vendor-onboarding') return 'ocr'
    if (app === 'dealer-a-billing')           return 'transactions'
    return 'transactions'
}

export default function DealerAPage() {
    ...
    if (id?.startsWith('p1.') || app === 'dealer-a-bills') return <Shell><F1_p1_OcrIngest /></Shell>
    ...
    if (id?.startsWith('p2.') || app === 'dealer-a-vendor-onboarding') return <Shell><F2_p1_W9Intake /></Shell>
    ...
    if (id?.startsWith('p3.') || app === 'dealer-a-billing') return <Shell><F3_p1_BillingAlert /></Shell>
    ...
    if (id?.startsWith('p4.') || app === 'dealer-a-order-po') return <Shell><F4_p1_PifPreview /></Shell>
    ...
    if (id?.startsWith('p5.') || app === 'dealer-a-ack') return <Shell><F5_p1_AckIngest /></Shell>
...
const STEP_TO_AC_EVENT: Record<string, string> = {
    'p1.1': 'dealer-a:bills-open',
    'p2.1': 'dealer-a:w9-open',
    'p2.2': 'dealer-a:w9-registered', // F84.16 · replaces custom overlay in F2_p2
    'p3.1': 'dealer-a:billing-open',
    'p4.1': 'dealer-a:pif-open',
    'p5.1': 'dealer-a:ack-open',
}
...
const SCENE_HANDLED_EVENTS = new Set<string>([
    'dealer-a:w9-open',   // F2 p2.1 · OCR list → notif → DocumentReviewModal opens in-place
])
...
function Shell({ children }: { children: React.ReactNode }) {
    ...
    return (
        <DealerAExperienceShell experience={experienceOf(app)} activeTab={activeTabFor(app)}>
            {children}
        </DealerAExperienceShell>
    )
}
