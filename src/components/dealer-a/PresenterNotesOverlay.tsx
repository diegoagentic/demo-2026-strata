 ...
 *          questions + what-to-click-next para el current Dealer A step. Reduce
 ...
 *          renderea (checks isDealerA + isDemoActive + notes disponibles para
 *          el current step).
 */
...
import { DEALER_A_PRESENTER_NOTES } from '../../config/profiles/dealer-a'
import { usePersistedToggle } from './hooks/usePersistedToggle'

export default function PresenterNotesOverlay() {
    const { activeProfile } = useDemoProfile()
    const { currentStep, isDemoActive } = useDemo()
    const [visible, setVisible] = usePersistedToggle('dealer-a-presenter-mode', false, 'presenter')
    ...
    // Guards · solo Dealer A profile · solo con demo activo (los notes son
    // per-step) · solo si hay notes para el step actual
    if (activeProfile.id !== 'dealer-a') return null
    if (!isDemoActive || !currentStep) return null
    const notes = DEALER_A_PRESENTER_NOTES[currentStep.id]
