                    ...
                    >
                        ...
                        <DialogPanel className="w-full max-w-2xl rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
                            ...
                            <div className="max-h-[70vh] overflow-y-auto">
                                ...
                                {/* F86.24.2 · Diego 2026-08-27 · destination radio moved to
                                    below the "Ready to release" list and made sticky to the
                                    bottom of the scroll area · always visible above the footer
                                    once the Approver has reviewed the items. See render at the
                                    end of this scroll container. */}
                                ...
                                {/* F86.24.2 · destination radio · sticky to bottom of the
                                    scroll pane so it stays visible right above the footer
                                    once the Approver scrolls through the list. `sticky` +
                                    opaque bg-card + z-10 · gated on canRelease so it stays
                                    hidden when there's nothing to release yet. */}
                                {preflightBreakdown.pendingCount > 0 && canRelease && (
                                    <div className="sticky bottom-0 z-10 px-5 py-3 border-t border-border bg-card space-y-3 shadow-[0_-4px_12px_-6px_rgba(0,0,0,0.08)]">
                                        <div className="flex items-start gap-2.5">
                                            <AlertTriangle className="h-4 w-4 text-warning shrink-0 mt-0.5" aria-hidden="true" />
                            ...
                            {/* Footer · F86.24.2 · inline "why disabled" hint moved out of the
                                tooltip and into a visible line so the Approver never has to
                                hover to understand why Confirm won't fire. */}
                            <div className="border-t border-border px-5 py-3 flex items-center gap-2 bg-muted/20">
                                ...
                                {(() => {
                                    ...
                                    const reason = preflightBreakdown.heldBills.length > 0
                                        ? `Resolve ${preflightBreakdown.heldBills.length} held item${preflightBreakdown.heldBills.length === 1 ? '' : 's'} above to continue`
                                        : !canRelease
                                            ? 'Approve at least one bill to enable release'
                                            : needsDestination
                                ...
                                {(() => {
                                    // F86.24.1 · needsDestination only meaningful when canRelease
                                    // is already true · avoids stacking a phantom disable reason
                                    // when the more fundamental "nothing approved" gate is active.
                                    const needsDestination = canRelease && preflightBreakdown.pendingCount > 0 && pendingDestination === null
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={disabled}
                                            title={title}
                                            className="inline-flex items-center gap-1.5 text-xs font-bold px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
                                        >
                                            <Send className="h-3.5 w-3.5" aria-hidden="true" />
                                            Confirm release
                                        </button>
